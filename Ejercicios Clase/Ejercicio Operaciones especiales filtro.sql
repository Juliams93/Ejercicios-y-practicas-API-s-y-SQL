...
-- 2 Selecciona los nombres con alias 'nombre' y apellidos, con alias 'Apellidos' 
...
contact_last_name AS Apeliidos
FROM customers;


-- 3 Selecciona todos los nombres con alias 'nombre' y apellidos, con alias 'Apellidos'
-- tanto de los clientes como de los empleados de las tablas employees y customers.
...
SELECT
employee_number AS NumeroEmpleado,
first_name AS NombreEmpleado,
last_name AS ApellidoEmpleado
FROM employees
WHERE employee_number IN(1002, 1076, 1088, 1612);


-- 5 Queremos ver ahora la 'ciudad' y los nombres de las empresas como 'nombre de la empresa ' de la tabla customers, que no estén en: 
-- Ireland, France, Germany.

SELECT
city AS Ciudad,
customer_name AS NombreEmpresa
FROM customers
...
SELECT 
customer_name AS NombreCliente,
city AS Ciudad
FROM customers
WHERE city LIKE '%__on' AND LENGTH(city) = 4;
...
SELECT
customer_name AS NombreCliente,
address_line1 AS PrimeraDireccion,
city AS Ciudad
FROM customers
WHERE address_line1 LIKE '%3%';


-- 9 Encuentra el nombre del cliente, primera dirección y ciudad de aquellas ciudades que contengan el número 3 en su dirección postal y 
-- la ciudad no empiece por T.
